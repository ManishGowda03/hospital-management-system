...
if (
  ...
  patient_id.startsWith("pediatric-")
) {
  actualPatientId = patient_id.replace("pediatric-", "");
}

if (patient_type === "Pediatric") {
  patient = await PediatricModel.findById(actualPatientId);
} else {
  patient = await PatientModel.findById(actualPatientId);
}
